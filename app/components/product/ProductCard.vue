<template>
  ...
  >
    ...
    <div class="relative h-64 overflow-hidden flex items-center justify-center">
      ...
      <div
        class="absolute inset-0 opacity-20 group-hover:opacity-30 transition-opacity"
        :style="{ background: `linear-gradient(135deg, #0f172a22, #1e3a5f22)` }"
      >
        <div
    ...
    <div class="p-6 flex flex-col flex-1 gap-4 relative">
      <div
        v-if="totalCount > 1"
        class="absolute py-2 text-center text-sm text-muted-foreground"
      ...
      <div class="mt-auto flex items-end justify-between">
        <div class="flex flex-col">
          <span
            v-if="product.price"
            class="text-xs text-muted-foreground/60 line-through mb-0.5"
          >
            {{ product.price }}
          </span>
          <span
