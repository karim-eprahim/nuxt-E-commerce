...
<template>
  <header class="sticky top-0 z-50 w-full border-b shadow-lg bg-white">
    <div class="mx-auto flex h-16 max-w-screen-2xl items-center gap-6 px-4 lg:px-8">
      ...
      <div class="flex-1 max-w-xs lg:max-w-sm xl:max-w-md">
        <div class="relative">
          <Search class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2  pointer-events-none" />
      ...
      <div class="flex items-center gap-1 ml-auto shrink-0">
        ...
        <!-- Auth Buttons -->
        <div v-if="!user">
          <NuxtLink to="/login" class="px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200">
            Sign in
          </NuxtLink>
        ...
        <DropdownMenu v-else>
          ...
          <DropdownMenuContent side="top">
            <DropdownMenuItem>
              <NuxtLink to="/account" class="flex items-center gap-2 text-foreground w-full h-full cursor-pointer">
                <Icon name="lucide:user" />
                <span>Account</span>
              </NuxtLink>
            </DropdownMenuItem>
            <DropdownMenuItem @click="logout" class="text-red-500 hover:text-red-600 hover:bg-red-500/20 cursor-pointer">
              <button class="flex items-center gap-2 text-red-500 w-full h-full">
