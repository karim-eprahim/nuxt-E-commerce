<template>
  <div
    class="group flex flex-col rounded-2xl border border-border/40 bg-card/40 backdrop-blur-md overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:border-indigo-500/30 hover:shadow-[0_30px_70px_rgba(0,0,0,0.4),0_0_0_1px_rgba(99,102,241,0.1)]"
  >
    <!-- Image Section -->
    <div
      class="relative h-64 overflow-hidden flex items-center justify-center"
      :style="{ background: `linear-gradient(135deg, ${product.gradient[0]}22, ${product.gradient[1]}22)` }"
    >
    ...
    <div class="p-6 flex flex-col flex-1 gap-4 relative">
      <div v-if="totalCount > 1" class="absolute py-2 text-center text-sm text-muted-foreground" style="transform: translate(300%, -52px);">
        <ul class="flex items-center gap-2">
          <li v-for="(_, index) in totalCount" :key="index">
            <div class="w-4 h-4 rounded-full border cursor-pointer" @click="scrollToImage(index)" :class="current === index + 1 ? 'bg-indigo-500' : ''" />
          </li>
        </ul>
      </div>
      <div>
        <h3
          class="text-lg font-bold text-foreground group-hover:text-indigo-400 transition-colors duration-300 mb-1"
        >
          {{ product.name }}
        </h3>
      ...
      <div class="mt-auto flex items-end justify-between">
        <div class="flex flex-col">
          <span v-if="product.originalPrice" class="text-xs text-muted-foreground/60 line-through mb-0.5">
